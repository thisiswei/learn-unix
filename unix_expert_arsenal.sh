...
process_forensics() {
    ...
    echo
    echo -e "${CYAN}# Dump process memory${NC}"
    ...
    echo
    echo -e "${CYAN}# Extract strings from process memory${NC}"
    ...
    echo
    echo -e "${CYAN}# System call tracing with filters${NC}"
    ...
    echo
    echo -e "${CYAN}# Library call tracing${NC}"
...
kernel_operations() {
    ...
    echo
    echo -e "${CYAN}# Module operations${NC}"
    ...
    echo
    echo -e "${CYAN}# Runtime kernel tuning${NC}"
    ...
    echo
    echo -e "${CYAN}# Kernel debugging${NC}"
    ...
    echo
    echo -e "${CYAN}# Raw device access${NC}"
    ...
    echo
    echo -e "${CYAN}# BPF tracing (requires bpftrace)${NC}"
    ...
    echo
    echo -e "${CYAN}# Trigger kernel operations${NC}"
...
filesystem_internals() {
    ...
    echo
    echo -e "${CYAN}# Inode and block analysis${NC}"
    ...
    echo
    echo -e "${CYAN}# Extended attributes${NC}"
    ...
    echo
    echo -e "${CYAN}# Advanced mount options${NC}"
    ...
    echo
    echo -e "${CYAN}# Custom filesystems${NC}"
    ...
    echo
    echo -e "${CYAN}# Direct block device manipulation${NC}"
...
network_analysis() {
    ...
    echo
    echo -e "${CYAN}# tcpdump wizardry${NC}"
    ...
    echo
    echo -e "${CYAN}# Namespace manipulation${NC}"
    ...
    echo
    echo -e "${CYAN}# Complex firewall rules${NC}"
    ...
    echo
    echo -e "${CYAN}# Advanced traffic shaping${NC}"
...
performance_profiling() {
    ...
    echo
    echo -e "${CYAN}# perf - Linux profiling tool${NC}"
    ...
    echo
    echo -e "${CYAN}# System memory analysis${NC}"
    ...
    echo
    echo -e "${CYAN}# Block I/O tracing${NC}"
    ...
    echo
    echo -e "${CYAN}# File access patterns${NC}"
    ...
    echo
    echo -e "${CYAN}# Combined profiling${NC}"
    ...
    echo
    echo -e "${CYAN}# Dynamic tracing${NC}"
...
text_wizardry() {
    ...
    echo
    echo -e "${CYAN}# Complex AWK scripts${NC}"
    ...
    echo
    echo -e "${CYAN}# Multi-line and branching${NC}"
    ...
    echo
    echo -e "${CYAN}# PCRE and lookarounds${NC}"
    ...
    echo
    echo -e "${CYAN}# Binary file processing${NC}"
    ...
    echo
    echo -e "${CYAN}# jq - JSON processor${NC}"
    ...
    echo
    echo -e "${CYAN}# xmlstarlet - XML processor${NC}"
...
shell_internals() {
    ...
    echo
    echo -e "${CYAN}# Coprocesses${NC}"
    echo "coproc BC { bc; }"
    echo "echo '2^10' >&\${BC[1]}"
    echo "read result <&\${BC[0]}"
    echo
    echo -e "${CYAN}# Nameref variables${NC}"
    echo "declare -n ref=original_var"
    echo "ref=\"new value\"  # Changes original_var"
    echo
    echo -e "${CYAN}# Advanced parameter expansion${NC}"
    ...
    echo
    echo -e "${CYAN}# Advanced signal handling${NC}"
    ...
    echo
    echo -e "${CYAN}# Advanced I/O redirection${NC}"
    ...
    echo
    echo -e "${CYAN}# Debugging and profiling${NC}"
    ...
    echo
    echo -e "${CYAN}# Dynamic loading${NC}"
...
security_tools() {
    ...
    echo
    echo -e "${CYAN}# Advanced hashing${NC}"
    ...
    echo
    echo -e "${CYAN}# OpenSSL operations${NC}"
...
system_recovery() {
    ...
    echo
    echo -e "${CYAN}# Emergency repairs${NC}"
    ...
    echo
    echo -e "${CYAN}# Recover from backup superblock${NC}"
    ...
    echo
    echo -e "${CYAN}# chroot recovery${NC}"
    ...
    echo
    echo -e "${CYAN}# Reset root password${NC}"
    ...
    echo
    echo -e "${CYAN}# Network debugging${NC}"
    ...
    echo
    echo -e "${CYAN}# File recovery tools${NC}"
...
oneliner_hall() {
    ...
    echo
    echo -e "${CYAN}# Hardware info in one line${NC}"
    echo "echo \"CPU: \$(lscpu | grep 'Model name' | cut -d: -f2 | xargs) | RAM: \$(free -h | awk '/^Mem:/{print \$2}') | Disk: \$(df -h / | awk 'NR==2{print \$2}')\""
    echo
    echo -e "${CYAN}# All network connections grouped by state${NC}"
    echo "ss -tan | awk 'NR>1 {state[\$1]++} END {for(s in state) print s\": \"state[s]}'"
    echo
    
    echo -e "${BOLD}Process Management${NC}"
    echo
    echo -e "${CYAN}# Kill all processes of a user except current shell${NC}"
    echo "kill -9 \$(ps -u username -o pid= | grep -v \$\$)"
    echo
    echo -e "${CYAN}# Top 10 memory consuming processes with percentage${NC}"
    echo "ps aux | awk '{printf \"%-30s %s\\n\", \$11, \$4\"%\"}' | sort -k2 -rn | head -10"
    echo
    
    echo -e "${BOLD}File Operations${NC}"
    echo
    echo -e "${CYAN}# Find and remove files older than 30 days but keep directory structure${NC}"
    echo "find /path -type f -mtime +30 -delete -o -type d -empty -delete"
    echo
    echo -e "${CYAN}# Recursive search and replace in all files${NC}"
    echo "find . -type f -exec sed -i 's/old/new/g' {} +"
    echo
    echo -e "${CYAN}# Find duplicate files by MD5${NC}"
    echo "find . -type f -exec md5sum {} + | sort | uniq -w32 -d"
    echo
    
    echo -e "${BOLD}Network Magic${NC}"
    echo
    echo -e "${CYAN}# Serve current directory via HTTP on port 8000${NC}"
    echo "python3 -m http.server 8000 --bind 0.0.0.0"
    echo
    echo -e "${CYAN}# Monitor real-time network traffic by process${NC}"
    echo "nethogs -d 1"
    echo
    echo -e "${CYAN}# SSH tunnel for port forwarding${NC}"
    echo "ssh -L 8080:localhost:80 -N -f user@remotehost"
    echo
    
    echo -e "${BOLD}Log Analysis${NC}"
    echo
    echo -e "${CYAN}# Top 10 IP addresses in Apache/Nginx logs${NC}"
    echo "awk '{print \$1}' access.log | sort | uniq -c | sort -rn | head -10"
    echo
    echo -e "${CYAN}# Real-time log monitoring with highlighting${NC}"
